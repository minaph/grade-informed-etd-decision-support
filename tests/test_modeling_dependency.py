...
from modeling_dependency import MODELING_PATH, package_files, validate_modeling_dependency
...
class ModelingDependencyTests(unittest.TestCase):
    def setUp(self):
        ...
        self.git(self.root, "-c", "protocol.file.allow=always", "submodule", "add", str(self.source), MODELING_PATH.as_posix())
        self.child = self.root / MODELING_PATH
    ...
    def test_initialized_pinned_child_and_parent_manifest(self):
        self.assertEqual(validate_modeling_dependency(self.root), [])
    ...
    def test_missing_initialization(self):
        self.git(self.root, "submodule", "deinit", "--force", "--", MODELING_PATH.as_posix())
        self.assertTrue(any("not initialized" in e for e in validate_modeling_dependency(self.root)))

    def test_dirty_and_untracked_child(self):
        (self.child / "guide.md").write_text("changed")
        self.assertTrue(any("uncommitted" in e for e in validate_modeling_dependency(self.root)))
        self.git(self.child, "restore", "guide.md")
        (self.child / "extra.md").write_text("new")
        self.assertTrue(any("untracked" in e for e in validate_modeling_dependency(self.root)))
    ...
    def test_wrong_commit(self):
        ...
        self.assertTrue(any("differs" in e for e in validate_modeling_dependency(self.root)))

    def test_child_reference_escape_and_missing_file(self):
        (self.child / "guide.md").write_text("[Outside](../../outside.md)\n[Missing](absent.md)\n")
        errors = validate_modeling_dependency(self.root)
    ...
    def test_child_frontmatter(self):
        ...
        self.assertTrue(any("matching name" in e for e in validate_modeling_dependency(self.root)))

    def test_gitlink_required(self):
        self.git(self.root, "rm", "--cached", MODELING_PATH.as_posix())
        self.assertTrue(any("pinned git submodule" in e for e in validate_modeling_dependency(self.root)))

    def test_clone_registration_required(self):
        (self.root / ".gitmodules").write_text("")
        self.assertTrue(any("clone URL" in e for e in validate_modeling_dependency(self.root)))
